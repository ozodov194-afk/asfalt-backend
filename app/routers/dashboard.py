...
def dashboard_segodnya(den: date = None, db: Session = Depends(get_db)):
    ...
    ).filter(cast(models.PrikhodSyrya.data_vremya, Date) == den).first()
    ...
    ).filter(cast(models.Proizvodstvo.data_vremya, Date) == den).first()
    ...
    ).filter(cast(models.Prodazha.data_vremya, Date) == den).first()

    # Приход по видам сырья
    prikhod_vidy = (
    ...
    return {
        "den": str(den),
        "prikhod": {
            "itogo_kg": round(prikhod[0], 1),
            "mashin": prikhod[1],
            "po_vidam": [{"vid": r.name, "kg": round(r.kg, 1), "mashin": r.mashin} for r in prikhod_vidy]
        },
        "raskhod": {
            "po_vidam": [{"vid": r.name, "kg": round(r.kg, 1)} for r in raskhod_rows]
        },
        "proizvodstvo": {
            "itogo_kg": round(proizvod[0], 1),
            "partiy": proizvod[1],
            "po_markam": [{"marka": r.name, "kg": round(r.kg, 1)} for r in prod_marki]
        },
        "prodazhi": {
            "itogo_kg": round(prodazhi[0], 1),
            "vyruchka": round(prodazhi[1], 0),
            "pokupatelei": prodazhi[2]
        },
